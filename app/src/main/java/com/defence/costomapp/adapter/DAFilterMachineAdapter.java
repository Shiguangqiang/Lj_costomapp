...
public class DAFilterMachineAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public DAFilterMachineAdapter(List<DataAnMachineFilterBean.MachineListBean> list, String type) {
        if (list == null) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        ...
        return new DAFilterMachineAdapter.ListItemViewHolder(itemView);
    }
        ...
        if (string.equals("machine")) {
            ...
            if (checkshop != null && checkshop.size() > 0) {
                for (int j = 0; j < checkshop.size(); j++) {
                    if (checkshop.get(j).equals(mList.get(i).getMachinenumber())) {
